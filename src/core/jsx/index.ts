...
import { escapeToBuffer, stringBufferToString } from "./utils";
import type { StringBuffer, HtmlEscaped, HtmlEscapedString } from "./utils";
    ...
    Provider(props): HtmlEscapedString | Promise<HtmlEscapedString> {
      ...
      if (string instanceof Promise) {
        return Promise.resolve().then<HtmlEscapedString>(async () => {
          ...
          const promiseRes = raw(
            awaited,
            (awaited as HtmlEscapedString).promises,
          );
          values.pop();
          return promiseRes;
        });
